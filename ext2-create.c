...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_used_dirs_count = 2; // root and lost+found dir
...
void write_block_bitmap(int fd) {
	/* Create an array where each index represents a byte */
	unsigned char *block_byte_array_ptr;
	unsigned char block_byte_array[1024];

	block_byte_array[0] = 0xFF;
	block_byte_array[1] = 0xFF;
	block_byte_array[2] = 0x7F;

	for (int i = 3; i <= 126; i++) {
		block_byte_array[i] = 0x00;
	}

	block_byte_array[127] = 0x80;

	for (int i = 128; i <= 1023; i++) {
		block_byte_array[i] = 0xFF;
	}

	block_byte_array_ptr = block_byte_array;
	...
	/* Write to the block bitmap */
	if (write(fd, block_byte_array_ptr, BLOCK_SIZE) != BLOCK_SIZE) {
		errno_exit("write");
	}
}
...
void write_inode_bitmap(int fd) {
	/* Create an array where each index represents a byte */
	unsigned char *inode_byte_array_ptr;
	unsigned char inode_byte_array[1024];

	inode_byte_array[0] = 0xFF;
	inode_byte_array[1] = 0x1F;

	for (int i = 2; i <= 15; i++) {
		inode_byte_array[i] = 0x00;
	}

	for (int i = 16; i <= 1023; i++) {
		inode_byte_array[i] = 0xFF;
	}

	inode_byte_array_ptr = inode_byte_array;
	...
	/* Write to the inode bitmap */
	if (write(fd, inode_byte_array_ptr, BLOCK_SIZE) != BLOCK_SIZE) {
		errno_exit("write");
	}
}
...
void write_inode_table(int fd) {
	...
	memcpy(&hello_inode.i_block, "hello-world", 12); //memcpy instead of allocating new block 
...
void write_hello_world_file_block(int fd) {
	/* get offset and write output to fd */
